...
volatile int ISR_pwm1 = 100, ISR_pwm2 = 60, ISR_cnt = 0;
...
void ConfigurePins(void)
{
	...
	ANSELB &= ~(1 << 6); // Set RB5 as a digital I/O (pin 15 of DIP28)
	...
	TRISBbits.TRISB10 = 0; // pin 14 of DIP28
	TRISBbits.TRISB12 = 0; // pin 14 of DIP28
	INTCONbits.MVEC = 1;
}
...
void main(void)
{
	...
	waitms(1000); // Give PuTTY time to start
	...
	while (counter < 20)
	{
		...
		if (adcval1 > 15 || adcval2 > 15)
		{
			stop();
			cadcval = adcval1 + adcval2;
			if (adcval1 % 2 == 0)
			{
				flag = 1;
			}
		...
		uart_puts(" adc1, count= ");
		PrintNumber(adcval1, 10, 6);
		uart_puts(" adc2, count= ");
		PrintNumber(adcval2, 10, 6);
		uart_puts("\r");
		if (count < 33500)
		{
			LATAbits.LATA2 = 1;
